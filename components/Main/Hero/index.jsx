  ...
  return (
    <div className='w-[100vw] transition-all duration-300 items-center h-[50vh] flex flex-col justify-between ' style={{
      ...
      <div className='w-[90vw]'>
        ...
        }} className={`${poppinsBold.className} transition-all duration-300 text-[38px] md:text-[56px] tracking-tight font-bold mb-[2rem] text-white`}>
          {selectedAnime.title.charAt(0).toUpperCase() + selectedAnime.title.slice(1)}
        </p>

        <div className=' mb-[-3rem] gap-[40px] hidden md:flex'>
          {
            topAnimes?.slice(0, 8).map((anime, index) => {
              return (
                ...
                }} className={`w-[200px] flex items-center justify-center cursor-pointer transition-all duration-300  ${selectedAnime?.id === anime?.id ? 'border-red-400 border-[2px] scale-110' : 'opacity-50'} h-[120px]  rounded-[20px]`}>
                  {
                    selectedAnime.id === anime.id && (
                      <div onClick={() => {
                        ...
                        <AiFillYoutube className='text-[#a83535] scale-[3]' />
                      </div>
                    )
                  }
                </div>
